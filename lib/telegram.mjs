...
function buildLead(fields, folio) {
  ...
  if (ft) {
    ...
    row('📧', 'Correo completo', fields.correo_completo);
    row('🔄', 'Cambio de correo (UGI)', fields.cambio_correo_ugi);
    row('✏️', 'Cambio de nombre', fields.cambio_nombre);
    row('⏪', 'Historial de recuperación / revert', fields.historial_recuperacion);
    row('🖥️', 'Plataformas vinculadas', fields.plataformas_vinculadas);
    row('🔗', 'Puede desvincular', fields.puede_desvincular);
    row('🔐', 'Respuestas de recuperación', fields.acceso_respuestas);
    row('🚫', 'Bloqueos o restricciones', fields.bloqueos);
  } else {
...
function buildAnuncioCaption(cuenta, siteUrl) {
  ...
  const L = [];
  L.push(juegoLabel);
  L.push(`<b>${esc(cuenta.titulo || codigo)}</b>`);
  L.push('');
  ...
  if (ft) {
    ...
    if (cuenta.cambio_nombre) L.push('✏️ <b>Cambio de nombre:</b> se puede');
    // Solo lo público: origen, inversion y demás datos del trato no salen de /admin.
    row('🔗', 'Plataformas vinculadas', cuenta.plataformas_vinculadas);
    if (/^s[ií]/i.test(cuenta.puede_desvincular || '')) L.push('🔓 <b>Desvinculación:</b> se pueden desvincular');
  } else {
  ...
  // El correo solo se menciona si se incluye (se omite "Sin correo original").
  if (cuenta.correo && cuenta.correo !== 'Sin correo original') row('📧', 'Correo', cuenta.correo);
  const extras = [];
  ...
  if (cuenta.link) L.push(`🔎 <b>Link de Explorant:</b> <a href="${esc(cuenta.link)}">abrir</a>`);
...
export async function anunciarCuentaTelegram(cuenta, env = process.env) {
  // Los anuncios pueden usar un bot/canal propio (ej. un canal público de la
  // tienda), independiente del chat que recibe los leads del formulario.
  const token = env.TELEGRAM_ANUNCIOS_BOT_TOKEN || env.TELEGRAM_BOT_TOKEN;
  ...
  // Con fotos: primero el mensaje con los detalles y DEBAJO el álbum de fotos.
  if (imgs.length > 0) {
    ...
    if (!msgJson.ok) throw new Error('Telegram sendMessage (anuncio): ' + (msgJson.description || 'error'));

    const media = imgs.map((url) => ({ type: 'photo', media: url }));
