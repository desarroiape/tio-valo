...
export async function anunciarCuenta(cuenta) {
  ...
  if (juego === 'valorant') {
    ...
    if (cuenta.cambio_nombre) fields.push({ name: '✏️ Cambio de nombre', value: 'Se puede', inline: true });
    // Solo lo público: origen, inversion y demás datos del trato no salen de /admin.
    if (cuenta.plataformas_vinculadas) fields.push({ name: '🔗 Plataformas vinculadas', value: String(cuenta.plataformas_vinculadas), inline: true });
    if (/^s[ií]/i.test(cuenta.puede_desvincular || '')) fields.push({ name: '🔓 Desvinculación', value: 'Se pueden desvincular', inline: true });
  }
  ...
  // como una galería DEBAJO del embed principal, todo en UN solo mensaje.
  // Máx 9 fotos: se deja hueco para el embed de "más información" al final
  // (Discord permite 10 embeds por mensaje).
  const imgs = (Array.isArray(cuenta.imagenes) ? cuenta.imagenes : []).filter(Boolean).slice(0, 9);
  const embeds = [embed];
  ...
  // Embed final (sin url propia) -> se muestra DEBAJO de las fotos, con el
  // mensaje y el canal donde pedir más información.
  const CANAL_INFO = 'https://discord.com/channels/1325290242848194632/1344401733354524712/1344403061027770489';
  embeds.push({
    description: `💬 ¿Quieres más información sobre esta cuenta? Entra a este canal: ${CANAL_INFO}`,
    color: NARANJA,
  });
