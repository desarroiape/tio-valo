...
export async function anunciarCuenta(cuenta) {
  ...
  const codigo = (juego === 'fortnite' ? 'FN-' : 'VAL-') + String(cuenta.id).padStart(3, '0');
  ...
  if (cuenta.link) fields.push({ name: '🔎 Explorar nombre', value: String(cuenta.link), inline: false });
  ...
  // Descarga las fotos de la cuenta y las SUBE como adjuntos al post del foro.
  const imgs = (Array.isArray(cuenta.imagenes) ? cuenta.imagenes : []).filter(Boolean).slice(0, 10);
  const fd = new FormData();
  let n = 0, primeraFoto = null;
  for (const url of imgs) {
    try {
      ...
      if (n === 0) primeraFoto = nombre;
      n++;
    } catch { /* si una imagen falla, se omite */ }
  }
  // La primera foto también va como imagen destacada del embed.
  if (primeraFoto) embed.image = { url: 'attachment://' + primeraFoto };
