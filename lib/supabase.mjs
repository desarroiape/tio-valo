...
export async function listCuentas({ soloDisponibles = true, juego } = {}) {
  ...
  if (soloDisponibles) {
    // Una cuenta vendida se sigue mostrando 7 días (marcada como vendida) y luego desaparece.
    const cutoff = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    q += `&or=(estado.eq.disponible,and(estado.eq.vendida,vendida_en.gte.${encodeURIComponent(cutoff)}))`;
  }
