...
(function () {
  ...
  const WA_NUM = CFG.whatsapp || '51903350631';
  // Enlace de WhatsApp con el mensaje ya escrito para la cuenta que se está viendo.
  const waLink = (c) => {
    const precio = tienePrecio(c.precio) ? ` ($${c.precio} USD)` : '';
    const msg = `¡Hola! Me interesa la cuenta ${c.codigo} — ${c.titulo}${precio}. ¿Sigue disponible?`;
    ...
    overlayBody.innerHTML = `
      ...
      <div class="p-6 sm:p-8">
        ...
        <div class="mt-8 flex flex-col gap-3 sm:flex-row">
          <a href="${waLink(c)}" target="_blank" rel="noopener" class="cta flex flex-1 items-center justify-center gap-2 bg-red px-6 py-4 font-cond text-sm font-700 uppercase tracking-[0.14em] text-ink btn-red hover:bg-red-dim">
            <svg width="19" height="19" viewBox="0 0 24 24" fill="currentColor"><path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.46 1.32 4.96L2 22l5.25-1.38a9.9 9.9 0 004.79 1.22h.01c5.46 0 9.9-4.45 9.9-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0012.04 2zm0 1.67c2.2 0 4.27.86 5.83 2.42a8.2 8.2 0 012.41 5.82c0 4.54-3.7 8.24-8.25 8.24a8.23 8.23 0 01-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.19 8.19 0 01-1.26-4.38c0-4.54 3.7-8.24 8.26-8.24zm-4.6 4.4c-.22 0-.57.08-.87.4-.3.33-1.14 1.11-1.14 2.71s1.17 3.15 1.33 3.36c.16.22 2.25 3.44 5.46 4.69.76.33 1.36.52 1.82.67.77.24 1.47.21 2.02.13.62-.09 1.9-.78 2.17-1.53.27-.75.27-1.4.19-1.53-.08-.13-.3-.21-.62-.37-.33-.16-1.9-.94-2.2-1.05-.3-.11-.51-.16-.73.16-.22.33-.84 1.05-1.03 1.27-.19.22-.38.24-.7.08-.33-.16-1.37-.5-2.61-1.61-.97-.86-1.62-1.92-1.81-2.25-.19-.32-.02-.5.14-.66.15-.15.33-.38.49-.57.16-.2.21-.33.32-.55.11-.22.05-.41-.03-.57-.08-.16-.72-1.74-1-2.38-.26-.62-.53-.54-.73-.55l-.62-.01z"/></svg>
            Quiero comprar esta cuenta</a>
